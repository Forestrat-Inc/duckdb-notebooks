"""
Load January 2025 Data for All Exchanges - Dynamic Schema Version

This script loads all available data for January 2025 across all exchanges (LSE, CME, NYQ)
using dynamic table creation based on actual file schemas.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/january_load_dynamic_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class DynamicMultiExchangeLoader:
    """
    Dynamic loader that creates tables based on actual file schemas
    """
    ...
    def _create_schemas(self):
        ...
        try:
            ...
            self.db_manager.execute_sql("CREATE SCHEMA IF NOT EXISTS audit")
            
                         # Create audit tables with auto-incrementing ID
             audit_sql = """
             CREATE SEQUENCE IF NOT EXISTS audit.data_ingestion_log_seq;
             CREATE TABLE IF NOT EXISTS audit.data_ingestion_log (
                 log_id INTEGER PRIMARY KEY DEFAULT nextval('audit.data_ingestion_log_seq'),
                 exchange VARCHAR,
            ...
            """
            self.db_manager.execute_sql(audit_sql)
            self.logger.info("Schemas and audit tables created successfully")
    ...
    def load_single_file(self, exchange: str, data_date: date, stage: str = 'ingestion') -> dict:
        ...
        try:
            ...
            result['records_loaded'] = records_loaded
             ...
             (log_id, exchange, processing_stage, data_date, record_count, 
              processing_status, source_file)
             VALUES (nextval('audit.data_ingestion_log_seq'), '{exchange}', '{stage}', '{data_date}', {records_loaded}, 
                    'success', '{data_path}')
             """
            self.db_manager.execute_sql(audit_sql)
            ...
            try:
                audit_sql = f"""
                INSERT INTO audit.data_ingestion_log 
                (exchange, processing_stage, data_date, processing_status, error_message)
                VALUES ('{exchange}', '{stage}', '{data_date}', 'failed', '{str(e)[:500]}')
                """
                self.db_manager.execute_sql(audit_sql)
            except:
                pass
...
def main():
    """Load all January 2025 data for all exchanges using dynamic schema"""
    
    # Initialize the loader
    logger.info("Initializing Dynamic MultiExchangeLoader...")
    db_manager = DuckDBManager(database_path="./multi_exchange_data_lake.duckdb")
    loader = DynamicMultiExchangeLoader(db_manager)
